"""
Methods updating the meta file for tracking of already processed data
and processing the meta file
"""
import collections
...
class MetaProcess:
    ...
    """
    _date_format = MetaProcessFormat.META_DATE_FORMAT.value

    @staticmethod
    def update_meta_file(extract_date_list: list, meta_key: str, s3_bucket_meta: S3BucketConnector):
        """
        Updating the meta fiule with the processed Xetra dates and todays date as processed date

        :param: extract_date_list -> a list of dates that are extracted from the source
        :param: meta_key -> key of the meta file on the S3 buckets
    ...
    def return_date_list(first_date: str, meta_key: str, s3_bucket_meta: S3BucketConnector):
        """
        Creating a list of ddates based on the input first_date and the already processed
        dates in the meta file
