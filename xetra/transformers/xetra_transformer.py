...
and for configuring our source and target
"""
...
class XetraSourceConfig(NamedTuple):
    """
    Class for configuring our source data
...
class XetraTargetConfig(NamedTuple):
    """
    Class for configuring our target data
...
class XetraETL():
    """
    Class for doing the ETL processes with our data
    """
    ...
    def transform_report1(self, data_frame: pd.DataFrame):
        ...
        data_frame[self.trg_args.trg_col_ch_prev_day_clos] = data_frame\
            .sort_values(by=[self.src_args.src_col_date])\
                .groupby([self.src_args.src_col_isin])[self.trg_args.trg_col_op_price]\
                    .shift(1)
        data_frame[self.trg_args.trg_col_ch_prev_day_clos] = (
            data_frame[self.trg_args.trg_col_op_price] \
            - data_frame[self.trg_args.trg_col_ch_prev_day_clos]
            ) / data_frame[self.trg_args.trg_col_ch_prev_day_clos ] * 100
    ...
    def load(self, data_frame: pd.DataFrame):
        ...
        :param data_frame: Pandas DataFrame and input
        """
        # Creating target key
        target_key = (
